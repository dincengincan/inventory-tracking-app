...
const RequestSubmitPage = () => {
    ...
    const makeProductRequest = async () => {
        const settings = {
          method: "PUT",
          body: JSON.stringify({
            inventoryNumber: Number(state.selectedData.inventoryNumber) - Number(state.requestedNumber),
          }),
          headers: {
        ...
        try {
          const fetchResponse = await fetch(`https://5e9b1cde10bf9c0016dd1b23.mockapi.io/inventory/${state.selectedData.productId}`, settings);
    ...
    const addNewOrder = async () => {
        ...
        try {
            const fetchResponse = await fetch("https://5e9b1cde10bf9c0016dd1b23.mockapi.io/orders", settings);
            const data = await fetchResponse.json();
            if(data){
              console.log("success")
            }
